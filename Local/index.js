    ...
    (function() {
      ...
      async.each = function(arr, iterator2, callback) {
        ...
        _each(arr, function(x) {
          iterator2(x, only_once(function(err) {
            if (err) {
              callback(err);
              callback = function() {
              };
      ...
      var _asyncMap = function(eachfn, arr, iterator2, callback) {
        ...
        eachfn(arr, function(x, callback2) {
          iterator2(x.value, function(err, v) {
            results[x.index] = v;
            callback2(err);
          });
        }, function(err) {
      ...
      async.reduce = function(arr, memo, iterator2, callback) {
        async.eachSeries(arr, function(x, callback2) {
          iterator2(memo, x, function(err, v) {
            memo = v;
            callback2(err);
          });
        }, function(err) {
      ...
      async.sortBy = function(arr, iterator2, callback) {
        async.map(arr, function(x, callback2) {
          iterator2(x, function(err, criteria) {
            if (err) {
              callback2(err);
            } else {
      ...
      proto._getFfprobePath = function(callback) {
        ...
        async.waterfall([
          ...
          function(ffprobe, cb) {
            ...
            self2._getFfmpegPath(function(err, ffmpeg4) {
              if (err) {
                cb(err);
              } else if (ffmpeg4.length) {
                var name = utils.isWindows ? "ffprobe.exe" : "ffprobe";
                var ffprobe2 = path8.join(path8.dirname(ffmpeg4), name);
...
var require_follow_redirects = __commonJS({
  "node_modules/follow-redirects/index.js"(exports2, module2) {
    ...
    module2.exports = wrap({ http: http6, https: https6 });
...
// scripts/Local/index.ts
var index_exports = {};
__export(index_exports, {
  PATH: () => PATH,
  download: () => download2,
  getData: () => downloadVideoAndExtractMetadata2,
  navigate: () => navigate,
...
async function getFrameScreenShot(videoPath) {
  ...
  return await new Promise((res, rej) => {
    const tempThumbnailPath = import_path.default.join(tempDir, `temp_thumbnail_${v4_default()}.png`);
    (0, import_fluent_ffmpeg.default)(videoPath).on("end", () => {
      if (!import_fs.default.existsSync(tempThumbnailPath)) return rej("unfounded image");
      const data = import_fs.default.readFileSync(tempThumbnailPath);
      import_fs.default.unlinkSync(tempThumbnailPath);
      res(data.toString("base64"));
    }).on("error", (err) => {
      rej("Error generating thumbnail: " + err.message);
    }).screenshot({
      filename: import_path.default.basename(tempThumbnailPath),
      folder: import_path.default.dirname(tempThumbnailPath),
      timestamps: [time],
      fastSeek: true
    });
  });
...
var AppPrefix = "YoutubeDownloader";
function getOriginalFileName(filename) {
  const regex = new RegExp(
    `^${AppPrefix} - (.+?) v[\\w]+(?: \\d+-\\d+)?\\.\\w+$`
  );
  const match = filename.match(regex);
  return match ? `${match[1]}${import_path3.default.extname(filename)}` : filename;
}
...
// scripts/Local/valid.ts
var import_fs2 = __toESM(require("fs"));
var PATH = "local";
async function navigate(str) {
  try {
    if (!import_fs2.default.existsSync(str)) return null;
    if (import_fs2.default.statSync(str).isFile()) await getVideoInfo(str);
    return {
      path: PATH,
      id: encodeURI(str),
...
function getResizedQualitiesFromLabel(fromQuality) {
...
var DownloadBase = class extends import_events2.default {
  constructor({ downloadingState }) {
    ...
    this.curSize = import_fs4.default.existsSync(this.downloadingState.path) ? import_fs4.default.statSync(this.downloadingState.path).size : 0;
...
var FfmpegBase = class _FfmpegBase extends LinkDownloadBase {
  ...
  async continuoDownloading(command) {
    ...
    const transfer = new import_stream5.PassThrough();
  ...
  async converting() {
    ...
    await new Promise((res, rej) => {
      const command = (0, import_fluent_ffmpeg3.default)().input(tempPath).outputOptions("-movflags +faststart").outputOptions("-c copy").on("progress", (progress) => {
  ...
  async download(func) {
    ...
    if (this.downloadingState.continued && import_fs5.default.existsSync(this.downloadingState.path)) {
      ...
      await new Promise(async (res, rej) => {
        const command = (0, import_fluent_ffmpeg3.default)().input(this.link).seekInput(this.ffmpegData?.start ?? 0).format(format).on("end", res).on("error", rej);
...
var FfmpegResizeBase = class _FfmpegResizeBase extends FfmpegBase {
  ...
  async download(func) {
    ...
    if (this.downloadingState.continued && import_fs6.default.existsSync(this.downloadingState.path)) {
      ...
      await new Promise(async (res, rej) => {
        const command = (0, import_fluent_ffmpeg4.default)().input(this.link).on("error", rej).on("end", res).setStartTime(this.ffmpegData?.start || 0);
...
// scripts/link/valid.ts
var PATH2 = "link";

// scripts/link/data.ts
var DownloadAgent = new import_https5.default.Agent({
  rejectUnauthorized: false
  // (NOTE: this will disable client verification)
});
var DownloadInstance3 = axios_default.create({ httpsAgent: DownloadAgent });
async function getData(metadata, url2, fileName) {
  const videoFormat = import_path7.default.extname(fileName).slice(1);
  const videoSize = metadata.format.size;
  const videoDuration = metadata.format.duration;
  const videoStreams = metadata.streams;
  const videoStream = videoStreams.find(
    (stream4) => stream4.codec_type === "video"
  );
  const audioStream = videoStreams.find(
    (stream4) => stream4.codec_type === "audio"
  );
  let thumbnail = await getFrameScreenShot(url2);
  const videoTitle = import_path7.default.extname(import_path7.default.basename(fileName))[0];
  const orgQuality = videoStream && getQualityFromResolution(videoStream.width, videoStream.height);
  return {
    ...
    medias: {
      VIDEO: orgQuality && [
        ...
        ...await Promise.all(
          getResizedQualitiesFromLabel(orgQuality.label).map(async (quality) => ({
            previewLink: url2,
            container: videoFormat,
            environment: ["desktop"],
            id: `${url2}-${quality.label}`,
            data: {
              PATH: PATH2,
              previewLink: url2,
              data: {
            ...
            size: await FfmpegResizeBase.getEstimatedFileSize({
              link: url2,
              resize: quality.height
            }) || void 0
          }))
      ...
      AUDIO: audioStream && [
        {
          ...
          quality: 0,
          text: {
            str: `${videoFormat} - (${audioStream.codec_name?.toUpperCase()}) ${audioStream.bits_per_sample}kbps`
          },
          size: await FfmpegResizeBase.getEstimatedFileSize({
            link: url2,
            editData: {
      ...
      OTHERS: orgQuality && [
        ...
        ...await Promise.all(
          getResizedQualitiesFromLabel(orgQuality.label).map(async (quality) => ({
            ...
            container: videoFormat,
            environment: ["desktop"],
            id: `${url2}-videoOnly-${quality.label}`,
            data: {
              PATH: PATH2,
              previewLink: url2,
              data: {
    ...
    viewerUrl: url2,
    title: import_path7.default.basename(fileName)
  };
}

// scripts/Local/data.ts
async function getVideoData(filePath) {
  try {
    const metadata = await getVideoInfo(filePath);
    const fileName = getOriginalFileName(import_path8.default.basename(filePath));
    return {
...
function sliceArray(arr, startIndex, length = 20) {
...
async function downloadVideoAndExtractMetadata2(query) {
  if (!query.id) return null;
  const filePath = decodeURI(query.id);
  if (import_fs7.default.lstatSync(filePath).isDirectory()) {
    const videos = getVideos(filePath);
    for (let i = 0; i < videos.length; i++) {
      try {
        const result = await downloadVideoAndExtractMetadata2({
          ...query,
          id: import_path8.default.join(filePath, videos[i])
        });
  ...
  const folderDist = import_path8.default.dirname(filePath);
...
function getVideos(folderDist) {
  return import_fs7.default.readdirSync(folderDist).filter((v) => {
    try {
      import_fs7.default.lstatSync(import_path8.default.join(folderDist, v)).isFile() && isVideoFile(v);
    } catch (error) {
      return false;
    }
  });
}
async function getAllVideosData(folderDist, mainFile, limit = 20) {
  const files = getVideos(folderDist);
  const arr = sliceArray(
    files.filter((v) => import_path8.default.join(folderDist, v) != mainFile),
    files.findIndex((v) => import_path8.default.join(folderDist, v) == mainFile) || 0,
    limit
  );
  const data = [];
  for (let i = 0; i < arr.length; i++) {
    try {
      const g = await getVideoData(import_path8.default.join(folderDist, arr[i]));
      if (g)
        data.push({
          id: g.filePath,
...
// scripts/Local/download.ts
var LocalBaseDownload = class extends FfmpegResizeBase {
};
function download2(data) {
