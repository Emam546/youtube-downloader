...
var require_follow_redirects = __commonJS({
  "node_modules/follow-redirects/index.js"(exports2, module2) {
    ...
    module2.exports = wrap({ http: http5, https: https6 });
...
function getOriginalFileName(filename) {
  ...
  return match ? `${match[1]}${import_path2.default.extname(filename)}` : filename;
}
...
async function continueDownloading(org, continued, start, duration) {
  ...
  import_fs3.default.renameSync(continued, import_path3.default.basename(tempPath));
      ...
      AUDIO: audioStream && [
        {
          ...
          data: {
            ...
            title: `${import_path6.default.basename(fileName)} - audioOnly`
          },
          quality: 0,
          text: {
      ...
      OTHERS: orgQuality && [
        {
          ...
          data: {
            ...
            ftype: videoFormat,
            fquality: orgQuality.label,
            title: `${import_path6.default.basename(fileName)} videoOnly`
          },
          quality: orgQuality.height,
          text: {
...
async function getVideoData(filePath) {
  try {
    ...
    const fileName = getOriginalFileName(import_path7.default.basename(filePath));
    return {
...
async function downloadVideoAndExtractMetadata2(query) {
  ...
  if (import_fs6.default.lstatSync(filePath).isDirectory()) {
    const videos = getVideos(filePath);
    for (let i = 0; i < videos.length; i++) {
      try {
        return await downloadVideoAndExtractMetadata2({
          ...query,
          id: import_path7.default.join(filePath, videos[i])
        });
      } catch (error) {
        console.log(error);
      }
  ...
  const folderDist = import_path7.default.dirname(filePath);
...
function getVideos(folderDist) {
  return import_fs6.default.readdirSync(folderDist).filter(
    (v) => import_fs6.default.lstatSync(import_path7.default.join(folderDist, v)).isFile() && isVideoFile(v)
  );
}
async function getAllVideosData(folderDist, mainFile, limit = 20) {
  const files = getVideos(folderDist);
  const arr = sliceArray(
    files.filter((v) => import_path7.default.join(folderDist, v) != mainFile),
    files.findIndex((v) => import_path7.default.join(folderDist, v) == mainFile) || 0,
    limit
  );
  const data = [];
  for (let i = 0; i < arr.length; i++) {
    try {
      const g = await getVideoData(import_path7.default.join(folderDist, arr[i]));
      if (g)
        data.push({
          id: g.filePath,
