...
export function StatusLi({
  ...
  return (
    <li className={classNames(className, "flex")} {...props}>
...
export function DownloadedStatus() {
  ...
  if (downloaded) {
    ...
    if (fileSize)
      status = `${status} ${Math.min(
        100,
        (downloaded / fileSize) * 100
      ).toFixed(2)}%`;
  }
...
export function TimeLeftStatus() {
  ...
  if (fileSize && transferRate && downloaded) {
    const time = Math.min(
      (fileSize - downloaded) / transferRate,
      360 * 24 * 60 * 60
    );
...
export function ResumeStatus() {
  ...
  return (
    <StatusLi label="Resume capability">
      <p className="px-4">
        {status == true && `Yes`}
        {status == false && <span className="text-red-500">{"NO"}</span>}
        {status == undefined && "......"}
      </p>
    </StatusLi>
  );
