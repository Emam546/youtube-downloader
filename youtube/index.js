...
var require_get_youtube_id = __commonJS({
  "node_modules/get-youtube-id/index.js"(exports2, module2) {
    "use strict";
    (function(root, factory) {
      ...
      return function(url2, opts) {
        if (opts == void 0) {
          opts = { fuzzy: true };
        }
        if (/youtu\.?be/.test(url2)) {
          var i;
          var patterns = [
            /youtu\.be\/([^#\&\?]{11})/,
            // youtu.be/<id>
          ...
          for (i = 0; i < patterns.length; ++i) {
            if (patterns[i].test(url2)) {
              return patterns[i].exec(url2)[1];
            }
          }
          if (opts.fuzzy) {
            var tokens = url2.split(/[\/\&\?=#\.\s]/g);
    ...
    (function() {
      ...
      async.each = function(arr, iterator, callback) {
        ...
        _each(arr, function(x2) {
          iterator(x2, only_once(function(err) {
            if (err) {
              callback(err);
              callback = function() {
              };
      ...
      var _asyncMap = function(eachfn, arr, iterator, callback) {
        ...
        eachfn(arr, function(x2, callback2) {
          iterator(x2.value, function(err, v) {
            results[x2.index] = v;
            callback2(err);
          });
        }, function(err) {
      ...
      async.reduce = function(arr, memo, iterator, callback) {
        async.eachSeries(arr, function(x2, callback2) {
          iterator(memo, x2, function(err, v) {
            memo = v;
            callback2(err);
          });
        }, function(err) {
      ...
      async.sortBy = function(arr, iterator, callback) {
        async.map(arr, function(x2, callback2) {
          iterator(x2, function(err, criteria) {
            if (err) {
              callback2(err);
            } else {
      ...
      async.whilst = function(test2, iterator, callback) {
        if (test2()) {
          iterator(function(err) {
            if (err) {
              return callback(err);
            }
            async.whilst(test2, iterator, callback);
          });
        } else {
          callback();
        }
      };
      async.doWhilst = function(iterator, test2, callback) {
        iterator(function(err) {
          if (err) {
            return callback(err);
          }
          if (test2()) {
            async.doWhilst(iterator, test2, callback);
          } else {
            callback();
          }
        });
      };
      async.until = function(test2, iterator, callback) {
        if (!test2()) {
          iterator(function(err) {
            if (err) {
              return callback(err);
            }
            async.until(test2, iterator, callback);
          });
        } else {
          callback();
        }
      };
      async.doUntil = function(iterator, test2, callback) {
        iterator(function(err) {
          if (err) {
            return callback(err);
          }
          if (!test2()) {
            async.doUntil(iterator, test2, callback);
          } else {
            callback();
          }
        });
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFMPEG_PATH) {
              fs7.exists(process.env.FFMPEG_PATH, function(exists) {
                if (exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFPROBE_PATH) {
              fs7.exists(process.env.FFPROBE_PATH, function(exists) {
          ...
          function(ffprobe, cb) {
            ...
            self2._getFfmpegPath(function(err, ffmpeg6) {
              if (err) {
                ...
                var ffprobe2 = path7.join(path7.dirname(ffmpeg6), name);
                fs7.exists(ffprobe2, function(exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FLVMETA_PATH) {
              fs7.exists(process.env.FLVMETA_PATH, function(exists) {
          ...
          function(flvtool, cb) {
            ...
            if (process.env.FLVTOOL2_PATH) {
              fs7.exists(process.env.FLVTOOL2_PATH, function(exists) {
    ...
    module2.exports = function(proto) {
      proto.ffprobe = function() {
        ...
        this._getFfprobePath(function(err, path7) {
          ...
          var ffprobe = spawn(path7, ["-show_streams", "-show_format"].concat(options, src), { windowsHide: true });
          if (input.isStream) {
      ...
      proto.takeScreenshots = proto.thumbnail = proto.thumbnails = proto.screenshot = proto.screenshots = function(config, folder) {
        ...
        async.waterfall([
          ...
          function createDirectory(filenames, next) {
            fs7.exists(config.folder, function(exists) {
              if (!exists) {
                fs7.mkdir(config.folder, function(err) {
                  if (err) {
                    next(err);
                  } else {
                    next(null, filenames);
          ...
          for (var i2 = 0; i2 < count; i2++) {
            ...
            self2.output(path7.join(config.folder, filenames[i2])).frames(1).map(stream4);
            if (i2 > 0) {
...
var require_fluent_ffmpeg = __commonJS({
  "node_modules/fluent-ffmpeg/lib/fluent-ffmpeg.js"(exports2, module2) {
    ...
    util2.inherits(FfmpegCommand, EventEmitter3);
...
var require_follow_redirects = __commonJS({
  "node_modules/follow-redirects/index.js"(exports2, module2) {
    ...
    module2.exports = wrap({ http: http5, https: https5 });
...
var require_axios = __commonJS({
  "node_modules/axios/dist/node/axios.cjs"(exports2, module2) {
    ...
    var https__default = /* @__PURE__ */ _interopDefaultLegacy(https5);
    ...
    var extend2 = (a, b2, thisArg, { allOwnKeys } = {}) => {
      forEach2(b2, (val, key) => {
    ...
    var cookies = platform.hasStandardBrowserEnv ? (
      // Standard browser envs support document.cookie
      {
        write(name, value, expires, path7, domain, secure) {
          ...
          utils$1.isString(path7) && cookie.push("path=" + path7);
    ...
    function mergeConfig3(config1, config2) {
      ...
      const mergeMap = {
        ...
        headers: (a, b2) => mergeDeepProperties(headersToObject2(a), headersToObject2(b2), true)
      };
    ...
    isFetchSupported2 && ((res) => {
      ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type) => {
        !resolvers2[type] && (resolvers2[type] = utils$1.isFunction(res[type]) ? (res2) => res2[type]() : (_2, config) => {
...
// node_modules/youtube-searches/dist/utils.js
var require_utils2 = __commonJS({
  "node_modules/youtube-searches/dist/utils.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.cutAfterJS = exports2.between = void 0;
    var between = (haystack, left, right) => {
      let pos;
...
// node_modules/youtube-searches/dist/getData.js
var require_getData = __commonJS({
  "node_modules/youtube-searches/dist/getData.js"(exports2) {
    "use strict";
    ...
    function default_1(url2) {
      return __awaiter(this, void 0, void 0, function* () {
        const res = yield (0, axios_1.default)(url2);
        return {
          res,
...
// node_modules/youtube-searches/dist/main.js
var require_main = __commonJS({
  "node_modules/youtube-searches/dist/main.js"(exports2) {
    "use strict";
    var __awaiter = exports2 && exports2.__awaiter || function(thisArg, _arguments, P2, generator) {
      function adopt(value) {
        return value instanceof P2 ? value : new P2(function(resolve) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.MergeUrl = exports2.parseData = void 0;
    var getData_1 = __importDefault(require_getData());
    var querystring_1 = __importDefault(require("querystring"));
    function parseData(data) {
      const searchVideos = data.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents[0].itemSectionRenderer.contents;
      const returnedData = [];
      searchVideos.forEach((ele) => {
        var _a;
        if (!ele.videoRenderer)
          return;
        const videoId = ele.videoRenderer.videoId;
        const thumbnails = ele.videoRenderer.thumbnail.thumbnails;
        const title = ele.videoRenderer.title.runs.map(({ text }) => text);
        const time = (_a = ele.videoRenderer.lengthText) === null || _a === void 0 ? void 0 : _a.simpleText;
        returnedData.push({
          videoId,
          thumbnails,
          title,
    ...
    function MergeUrl(query) {
      return `https://www.youtube.com/results?${query}`;
    }
    exports2.MergeUrl = MergeUrl;
    exports2.default = (search, options) => __awaiter(void 0, void 0, void 0, function* () {
      const str = querystring_1.default.stringify(Object.assign({ search_query: search }, options));
      const { res, data } = yield (0, getData_1.default)(MergeUrl(str));
      if (res.status != 200)
...
// node_modules/youtube-searches/dist/index.js
var require_dist = __commonJS({
  "node_modules/youtube-searches/dist/index.js"(exports2) {
    "use strict";
    var __importDefault = exports2 && exports2.__importDefault || function(mod) {
      return mod && mod.__esModule ? mod : { "default": mod };
    };
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.GetData = exports2.MergeUrl = void 0;
    var main_1 = require_main();
    Object.defineProperty(exports2, "MergeUrl", { enumerable: true, get: function() {
      return main_1.MergeUrl;
    } });
    var main_2 = __importDefault(require_main());
    exports2.GetData = main_2.default;
  }
});

// scripts/youtube/index.ts
var index_exports = {};
...
function getTime(data, defaultN, maxTime) {
...
function youtube_parser(url2) {
  return (0, import_get_youtube_id.default)(url2);
}
function validateURL(string) {
  const id = (0, import_get_youtube_id.default)(string);
  return id != null;
}
...
// scripts/youtube/valid.ts
function appendPathToBaseUrl(...paths) {
  let baseUrl2 = "/";
  for (let i = 0; i < paths.length; i++) {
    const path7 = paths[i];
    if (!baseUrl2.endsWith("/")) baseUrl2 += "/";
    if (path7.startsWith("/")) baseUrl2 += path7.slice(1);
    else baseUrl2 += path7;
  }
  return baseUrl2;
}
var PATH = "youtube";
var baseUrl = `/${PATH}/`;
function getPathYoutubeUrl(youtubeUrl) {
  const [id, list, time] = extractParams(youtubeUrl);
  const url2 = id ? appendPathToBaseUrl(baseUrl, encodeURI(id)) : baseUrl;
...
function extractParams(youtubeUrl) {
  const id = youtube_parser(youtubeUrl.href);
  const list = youtubeUrl.searchParams.get("list");
  let start = youtubeUrl.searchParams.get("start") || youtubeUrl.searchParams.get("t");
...
function isYoutubeUrl(val) {
  ...
  return extractParams(new URL(val)).slice(0, 2).some((val2) => val2 != null);
}
function navigate(str) {
  if (isYoutubeUrl(str)) return getPathYoutubeUrl(new URL(str));
  return null;
}
function constructUrl(...[id, listId, time]) {
  const url2 = new URL("https://www.youtube.com/watch");
  if (id) url2.searchParams.set("v", id);
  if (listId && listId.startsWith("PL")) url2.searchParams.set("list", listId);
  if (time && time[0]) {
    url2.searchParams.set("t", `${time[0]}s`);
  }
  return url2;
}
...
var E = 'bright-{"status":"%(progress.status)s","downloaded":"%(progress.downloaded_bytes)s","total":"%(progress.total_bytes)s","total_estimate":"%(progress.total_bytes_estimate)s","speed":"%(progress.speed)s","eta":"%(progress.eta)s"}';
function D(e, r = 2) {
  let t = Number(e);
  if (t === 0 || isNaN(t)) return t + " Bytes";
  let s = 1024, i = r < 0 ? 0 : r, n = ["Bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"], a = Math.floor(Math.log(t) / Math.log(s));
  return parseFloat((t / Math.pow(s, a)).toFixed(i)) + " " + n[a];
}
function ee(e, r, t) {
...
function W(e, r) {
  return ee(100 * Number(e) / Number(r), 2);
}
function re(e) {
  e = Number(e);
  let r = Math.floor(e / 3600), t = Math.floor(e % 3600 / 60), s = Math.floor(e % 3600 % 60), i = r > 0 ? r + (r == 1 ? " hour, " : " hours, ") : "", n = t > 0 ? t + (t == 1 ? " minute, " : " minutes, ") : "", a = s >= 0 ? s + (s == 1 ? " second" : " seconds") : "";
  return i + n + a;
}
function m(e) {
  try {
    if (!e.includes("bright")) throw new Error();
    let r = e.split("\r")?.[1]?.trim()?.split("-")?.[1];
    if (!r) throw new Error();
    let t = JSON.parse(r), s = isNaN(Number(t.total)) ? Number(t.total_estimate) : Number(t.total);
    return { status: t.status, downloaded: Number(t.downloaded), downloaded_str: D(t.downloaded), total: s, total_str: D(s), speed: Number(t.speed), speed_str: D(t.speed) + "/s", eta: Number(t.eta), eta_str: re(t.eta), percentage: W(t.downloaded, s), percentage_str: W(t.downloaded, s) + "%" };
  } catch {
    return;
  }
}
function z(e, r = {}) {
  return new Promise((t, s) => {
    let a = (new import_url2.URL(e).protocol === "https:" ? te : se).get(e, r, (o) => {
      if (o.statusCode >= 300 && o.statusCode < 400 && o.headers.location) {
        let u = new import_url2.URL(o.headers.location, e).toString();
        z(u, r).then(t).catch(s);
        return;
      }
      t(o);
    });
    a.on("error", s), a.setTimeout(3e4, () => {
      a.destroy(), s(new Error("Request timed out"));
    });
  });
}
async function y(e, r) {
  try {
    let t = h.createWriteStream(r), s = await z(e);
    if (s.statusCode !== 200) throw t.close(), h.unlinkSync(r), new Error(`Failed to download file: ${s.statusCode} ${s.statusMessage}`);
    let i = parseInt(s.headers["content-length"] || "0", 10), n = 0;
    return s.on("data", (a) => {
      n += a.length;
      let o = n / i * 100;
      process.stdout.write(`Progress: ${Math.round(o)}%\r`);
    }), s.pipe(t), new Promise((a, o) => {
      t.on("finish", () => {
        t.close(), console.log(`
Download complete!`), a();
      }), t.on("error", (u) => {
        t.close(), h.unlinkSync(r), o(u);
      }), s.on("error", (u) => {
        t.close(), h.unlinkSync(r), o(u);
      });
    });
  } catch (t) {
    throw h.existsSync(r) && h.unlinkSync(r), t;
  }
}
var ie = "https://github.com/iqbal-rashed/ytdlp-nodejs/releases/download/ffmpeg-latest";
var _ = { win32: { x64: ["win-x64-ffmpeg.exe", "win-x64-ffprobe.exe"], ia32: ["win-ia32-ffmpeg.exe", "win-ia32-ffprobe.exe"], arm64: ["win-arm64-ffmpeg.exe", "win-arm64-ffprobe.exe"] }, linux: { x64: ["linux-x64-ffmpeg", "linux-x64-ffprobe"], arm64: ["linux-arm64-ffmpeg", "linux-arm64-ffprobe"] }, darwin: { x64: ["macos-x64-ffmpeg", "macos-x64-ffprobe"], arm64: ["macos-arm64-ffmpeg", "macos-arm64-ffprobe"] }, android: { arm64: ["linux-arm64-ffmpeg", "linux-arm64-ffprobe"] } };
function U() {
  let e = process.platform, r = process.arch;
  if (!_[e] || !_[e][r]) throw new Error(`No FFmpeg build available for platform: ${e}, architecture: ${r}`);
  return _[e][r];
}
async function C(e) {
  let r = e || g, t = x();
  ...
  try {
    ...
    let i = s.map((a) => `${ie}/${a}`), n = s.map((a) => import_path2.default.join(r, String(a.split("-").pop())));
    import_fs.default.existsSync(r) || import_fs.default.mkdirSync(r, { recursive: true }), console.log("Downloading FFmpeg and FFprobe...");
    for (let a = 0; a < s.length; a++) {
      let o = i[a], u = n[a];
      console.log("Downloading...", import_path2.default.basename(o)), await y(o, u);
    }
    try {
      for (let a of n) import_fs.default.chmodSync(a, 493);
    } catch {
...
function x() {
  try {
    ...
    let r = import_path2.default.join(g, String(e[0].split("-").pop()));
    if (!import_fs.default.existsSync(r)) throw new Error("FFmpeg binary not found. Please download it first.");
    return r;
  } catch {
    return;
  }
}
var S = { win32: { x64: "yt-dlp.exe", ia32: "yt-dlp_x86.exe" }, linux: { x64: "yt-dlp", armv7l: "yt-dlp_linux_armv7l", aarch64: "yt-dlp_linux_aarch64", arm64: "yt-dlp" }, darwin: { x64: "yt-dlp_macos", arm64: "yt-dlp_macos" }, android: { arm64: "yt-dlp" } };
function $() {
  let e = process.platform, r = process.arch;
  try {
    let t = S[e][r], s = N.join(g, t);
    if (!d.existsSync(s)) throw new Error("Ytdlp binary not found. Please download it first.");
    return s;
  } catch {
    return;
  }
...
var Q = class {
  constructor(r) {
    if (this.binaryPath = r?.binaryPath || $() || "", this.ffmpegPath = r?.ffmpegPath || x(), (!this.binaryPath || !F.existsSync(this.binaryPath)) && console.error(new Error("yt-dlp binary not found. Please install yt-dlp or specify correct binaryPath in options.")), this.ffmpegPath && !F.existsSync(this.ffmpegPath) && console.error(new Error(`FFmpeg binary not found at: ${this.ffmpegPath}. Please install FFmpeg or specify correct ffmpegPath.`)), process.platform !== "win32") try {
      F.chmodSync(this.binaryPath, 493);
    } catch (t) {
      console.error(new Error(`Failed to set executable permissions: ${t instanceof Error ? t.message : "Unknown error"}`));
    }
  }
  async checkInstallationAsync(r) {
    return new Promise((t, s) => {
      if (r?.ffmpeg && !this.ffmpegPath) return s(new Error("FFmpeg path is not set"));
      let i = (0, import_child_process.spawn)(this.binaryPath, ["--version"]), n = false, a = !r?.ffmpeg;
      i.on("error", () => n = false), i.on("exit", (o) => {
        if (n = o === 0, r?.ffmpeg) {
          let u = (0, import_child_process.spawn)(this.ffmpegPath, ["-version"]);
          u.on("error", () => a = false), u.on("exit", (f) => {
            a = f === 0, t(!!(n && a));
          });
        } else t(!!n);
      });
  ...
  checkInstallation(r) {
    ...
    let t = (0, import_child_process.spawnSync)(this.binaryPath, ["--version"], { stdio: "ignore" }), s = r?.ffmpeg ? (0, import_child_process.spawnSync)(this.ffmpegPath, ["-version"], { stdio: "ignore" }) : { status: 0 };
    return t.status === 0 && s.status === 0;
  }
  async execAsync(r, t) {
    let s = this.buildArgs(r, t || {}), i = (n) => {
      if (t?.onData?.(n), t?.onProgress) {
        let a = m(n);
        a && t.onProgress?.(a);
      }
    };
    return this._executeAsync(s, i);
  }
  exec(r, t) {
    let s = this.buildArgs(r, t || {}, true);
    return this._execute(s);
  }
  _execute(r) {
    let t = (0, import_child_process.spawn)(this.binaryPath, r);
    return t.stderr.on("data", (s) => {
      let i = Buffer.from(s).toString(), n = m(i);
      n && t.emit("progress", n);
    }), t.stdout.on("data", (s) => {
      let i = Buffer.from(s).toString(), n = m(i);
      n && t.emit("progress", n);
  ...
  async _executeAsync(r, t, s) {
    return new Promise((i, n) => {
      this.binaryPath || n(new Error("Ytdlp binary not found"));
      let a = (0, import_child_process.spawn)(this.binaryPath, r), o = "", u = "";
      a.stdout.on("data", (f) => {
        s || (o += f.toString(), t?.(Buffer.from(f).toString()));
      }), a.stderr.on("data", (f) => {
        u += f.toString(), t?.(Buffer.from(f).toString());
      }), s && a.stdout.pipe(s), a.on("close", (f) => {
        f === 0 ? i(o) : n(new Error(`yt-dlp exited with code ${f}: ${u}`));
      }), a.on("error", (f) => {
        n(new Error(`Failed to start yt-dlp process: ${f.message}`));
      });
    });
  }
  buildArgs(r, t, s, i) {
    let n = w(t);
    return this.ffmpegPath && n.push("--ffmpeg-location", this.ffmpegPath), s && n.push("--progress-template", E), i && n.push(...i), n.concat(r);
  }
  download(r, t) {
    let { format: s, ...i } = t || {}, n = this.buildArgs(r, i, true, b(s));
    return this._execute(n);
  }
  async downloadAsync(r, t) {
    let { format: s, onProgress: i, ...n } = t || {}, a = this.buildArgs(r, n, !!i, b(s));
    return this._executeAsync(a, (o) => {
      let u = m(o);
      u && i?.(u);
    });
  }
  stream(r, t) {
    let { format: s, onProgress: i, ...n } = t || {}, a = this.buildArgs(r, n, !!i, [...b(s), "-o", "-"]), o = new import_stream.PassThrough();
    return { promise: this._executeAsync(a, (f) => {
      let l = m(f);
      l && i?.(l);
    }, o), pipe: (f, l) => o.pipe(f, l), pipeAsync: (f, l) => new Promise((O, T) => {
      let v = o.pipe(f, l);
      f.on("finish", () => O(v)), f.on("error", T);
    }) };
  }
  async getInfoAsync(r, t) {
    let s = ["--dump-single-json", "--quiet", ...w({ flatPlaylist: true, ...t }), r], i = await this._executeAsync(s);
    return JSON.parse(i);
  }
  async getThumbnailsAsync(r) {
    let t = ["--print", "thumbnails_table", "--print", "playlist:thumbnails_table", "--quiet", r], s = await this._executeAsync(t);
    return R(s);
  }
  async getTitleAsync(r) {
    let t = ["--print", "title", r];
    return await this._executeAsync(t);
  }
  async downloadFFmpeg() {
    return C();
  }
  async getFileAsync(r, t) {
    let s = await this.getInfoAsync(r), { format: i, filename: n, metadata: a, onProgress: o, ...u } = t || {}, f = new import_stream.PassThrough(), l = [];
    f.on("data", (I) => l.push(Buffer.from(I)));
    let O = this.buildArgs(r, u, !!o, [...b(i), "-o", "-"]);
    await this._executeAsync(O, (I) => {
      if (o) {
        let j = m(I);
        j && o(j);
      }
    }, f);
    let T = new import_buffer.Blob(l, { type: k(i) }), v = { name: n || `${s.title}.${B(i)}`, type: k(i), size: T.size, ...a };
    return new File([Buffer.concat(l)], v.name, { type: v.type });
  }
  async getUrlsAsync(r, t) {
    let s = ["--print", "urls", ...w({ flatPlaylist: true, ...t }), r], i = await this._executeAsync(s);
...
function getVideoInfo(path7) {
  return new Promise((res, rej) => {
    import_fluent_ffmpeg.default.ffprobe(path7, (err, metadata) => {
      if (err) rej(err);
      res(metadata);
    });
  });
...
function getTempName(orgPath) {
  const dir = import_path3.default.dirname(orgPath);
  const baseName = import_path3.default.basename(orgPath, import_path3.default.extname(orgPath));
  const newFileName = `${baseName}.temp${import_path3.default.extname(orgPath)}`;
  return import_path3.default.join(dir, newFileName);
}
async function continueDownloading(org, continued, start, duration) {
  const tempPath = getTempName(continued);
  import_fs2.default.renameSync(continued, import_path3.default.basename(tempPath));
  ...
  return (0, import_fluent_ffmpeg2.default)().input(tempPath).inputOptions(`-t ${resumePoint}`).input(org).setStartTime(start + resumePoint).inputOptions(duration ? [`-t ${duration - resumePoint}`] : []).on("end", () => {
    import_fs2.default.unlinkSync(tempPath);
  });
}
...
async function continueDownloading2(org, continued, start, duration) {
  const tempPath = getTempName(continued);
  import_fs3.default.renameSync(continued, import_path4.default.basename(tempPath));
  const info = await getVideoInfo(tempPath);
  const outputDuration = info.format.duration;
  const resumePoint = outputDuration > 5 ? outputDuration - 5 : 0;
...
var cookies_default = platform_default.hasStandardBrowserEnv ? (
  // Standard browser envs support document.cookie
  {
    write(name, value, expires, path7, domain, secure) {
      ...
      utils_default.isString(path7) && cookie.push("path=" + path7);
...
var HttpDownloadAgent = new import_http3.default.Agent({});
...
var DownloadBase = class extends import_events2.default {
  constructor({ downloadingState, curSize }) {
    super();
...
var HttpDownloadAgent2 = new import_http5.default.Agent({});
...
// scripts/utils/Bases/ffmpeg/index.ts
import_fluent_ffmpeg4.default.setFfmpegPath(process.env.ffmpegPath);
import_fluent_ffmpeg4.default.setFfprobePath(process.env.ffmpegProbe);
...
function isFile2(link) {
  try {
    return import_fs4.default.statSync(link).isFile();
  } catch (error) {
    return false;
  }
}
var FfmpegBase = class _FfmpegBase extends LinkDownloadBase {
  ...
  async continuoDownloading(command) {
    ...
    commando.mergeToFile(transfer, import_path5.default.dirname(this.downloadingState.path));
    this.once("close", () => {
      try {
  ...
  async download(func) {
    ...
    const format = import_path5.default.extname(this.downloadingState.path).slice(1);
    this.setFileSize(
    ...
    if (this.downloadingState.continued && import_fs4.default.existsSync(this.downloadingState.path)) {
...
var FfmpegResizeBase = class _FfmpegResizeBase extends FfmpegBase {
  ...
  async continueDownResize(command) {
    ...
    commando.mergeToFile(
      this.downloadingState.path,
      import_path6.default.dirname(this.downloadingState.path)
    );
    this.once("close", () => {
      try {
        command.kill("SIGKILL");
  ...
  async download(func) {
    ...
    const format = import_path6.default.extname(this.downloadingState.path).slice(1);
    if (this.downloadingState.continued && import_fs5.default.existsSync(this.downloadingState.path)) {
      const command = (await continueDownloading(
        this.link,
        this.downloadingState.path,
        this.ffmpegData?.start || 0,
...
var FfmpegMergeBase = class _FfmpegMergeBase extends FfmpegResizeBase {
  ...
  async download(func) {
    ...
    const format = import_path7.default.extname(this.downloadingState.path).slice(1);
...
async function getYtdlpVideoData(url2) {
  const result = await ytdlp.execAsync(url2, {
    dumpJson: true,
...
async function getVideoData(query) {
  ...
  if (id == void 0 || !validateID(id)) return null;
  ...
  const duration = YtdpData.duration;
  const audios = [
  ...
  const others = [
    ...YtdpFormats.filter((v) => v.has_video && !v.has_audio).map((video) => {
      return {
        ...
        text: {
          str: `Video Only ${video.qualityLabel} (${video.vcodec?.toUpperCase()}) (.${video.ext})`
        },
...
// scripts/youtube/search.ts
var import_youtube_searches = __toESM(require_dist());
async function getSearchData(query) {
  const data = await (0, import_youtube_searches.GetData)(query);
  return [
    {
      ...
      data: data.resVideos.map((v) => {
        return {
          id: v.videoId,
          link: `/${PATH}/${v.videoId}`,
          thumbnail: v.thumbnails.sort((a, b2) => a.height - b2.height).at(-1).url,
          title: v.title,
          duration: 0
        };
      })
