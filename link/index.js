    ...
    module2.exports = function(proto) {
      proto.usingPreset = proto.preset = function(preset) {
        if (typeof preset === "function") {
          ...
          try {
            var modulePath = path6.join(this.options.presets, preset);
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFMPEG_PATH) {
              fs6.exists(process.env.FFMPEG_PATH, function(exists) {
                if (exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFPROBE_PATH) {
              fs6.exists(process.env.FFPROBE_PATH, function(exists) {
          ...
          function(ffprobe, cb) {
            ...
            self2._getFfmpegPath(function(err, ffmpeg4) {
              if (err) {
                ...
                var ffprobe2 = path6.join(path6.dirname(ffmpeg4), name);
                fs6.exists(ffprobe2, function(exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FLVMETA_PATH) {
              fs6.exists(process.env.FLVMETA_PATH, function(exists) {
          ...
          function(flvtool, cb) {
            ...
            if (process.env.FLVTOOL2_PATH) {
              fs6.exists(process.env.FLVTOOL2_PATH, function(exists) {
  ...
  "node_modules/fluent-ffmpeg/lib/ffprobe.js"(exports2, module2) {
    ...
    module2.exports = function(proto) {
      proto.ffprobe = function() {
        ...
        this._getFfprobePath(function(err, path6) {
          if (err) {
          ...
          var ffprobe = spawn(path6, ["-show_streams", "-show_format"].concat(options, src), { windowsHide: true });
          if (input.isStream) {
        ...
        async.waterfall([
          ...
          function fixPattern(next) {
            ...
            if (config.timemarks.length > 1 && !pattern.match(/%(s|0*i)/)) {
              var ext = path6.extname(pattern);
              pattern = path6.join(path6.dirname(pattern), path6.basename(pattern, ext) + "_%i" + ext);
            }
            next(null, pattern);
          },
          ...
          function replaceFilenameTokens(pattern, next) {
            if (pattern.match(/%[bf]/)) {
              ...
              pattern = pattern.replace(/%f/g, path6.basename(source)).replace(/%b/g, path6.basename(source, path6.extname(source)));
            }
            next(null, pattern);
          },
          ...
          function createDirectory(filenames, next) {
            fs6.exists(config.folder, function(exists) {
              if (!exists) {
                fs6.mkdir(config.folder, function(err) {
                  if (err) {
                    next(err);
                  } else {
                    next(null, filenames);
          ...
          for (var i2 = 0; i2 < count; i2++) {
            ...
            self2.output(path6.join(config.folder, filenames[i2])).frames(1).map(stream4);
            if (i2 > 0) {
...
var require_fluent_ffmpeg = __commonJS({
  "node_modules/fluent-ffmpeg/lib/fluent-ffmpeg.js"(exports2, module2) {
    ...
    function FfmpegCommand(input, options) {
      ...
      options.presets = options.presets || options.preset || path6.join(__dirname, "presets");
    ...
    FfmpegCommand.setFfmpegPath = function(path7) {
      new FfmpegCommand().setFfmpegPath(path7);
    };
    FfmpegCommand.setFfprobePath = function(path7) {
      new FfmpegCommand().setFfprobePath(path7);
    };
    FfmpegCommand.setFlvtoolPath = function(path7) {
      new FfmpegCommand().setFlvtoolPath(path7);
    };
...
// scripts/link/index.ts
var index_exports = {};
__export(index_exports, {
  PATH: () => PATH,
  download: () => download,
  getData: () => downloadVideoAndExtractMetadata,
  navigate: () => navigate,
...
function getVideoInfo(path6) {
  return new Promise((res, rej) => {
    import_fluent_ffmpeg.default.ffprobe(path6, (err, metadata) => {
      if (err) rej(err);
      res(metadata);
    });
  });
...
var DownloadBase = class extends import_events2.default {
  constructor({ downloadingState }) {
    ...
    this.curSize = import_fs3.default.existsSync(this.downloadingState.path) ? import_fs3.default.statSync(this.downloadingState.path).size : 0;
...
var FfmpegBase = class _FfmpegBase extends LinkDownloadBase {
  ...
  async continuoDownloading(command) {
    ...
    commando.mergeToFile(transfer, import_path4.default.dirname(this.downloadingState.path));
    this.once("close", () => {
      try {
  ...
  async download(func) {
    ...
    const format = import_path4.default.extname(this.downloadingState.path).slice(1);
    this.setFileSize(
...
var FfmpegResizeBase = class _FfmpegResizeBase extends FfmpegBase {
  ...
  async continueDownResize(command) {
    ...
    commando.mergeToFile(
      this.downloadingState.path,
      import_path5.default.dirname(this.downloadingState.path)
    );
    this.once("close", () => {
      try {
        command.kill("SIGKILL");
...
function isVideo(v) {
  return import_mime_types.default.contentType(v).toString().includes("video") || false;
}
async function downloadVideoAndExtractMetadata(query) {
  if (!query.link) return null;
  try {
    const url2 = decodeURI(query.link);
    const response = await DownloadInstance3.head(url2);
    const contentDisposition = response.headers["content-disposition"];
    const metadata = await getVideoInfo(url2);
    let reqFilename;
    ...
    const expectedVideoFormat = metadata.format.format_name?.split(",")[0] || "mp4";
    const urlFileName = decodeURI(url2.split("/").pop()?.split("?")[0] || "");
    const fileName = metadata.format.tags?.title || reqFilename || (isVideo(urlFileName) ? urlFileName : void 0) || `video.${expectedVideoFormat}`;
    return {
      video: await getData(metadata, url2, fileName)
    };
  } catch (error) {
    console.error(error);
    return null;
...
async function getData(metadata, url2, fileName) {
  const videoFormat = import_path6.default.extname(fileName).slice(1);
  ...
  const videoTitle = import_path6.default.extname(import_path6.default.basename(fileName))[0];
  ...
  return {
    ...
    viewerUrl: url2,
    title: import_path6.default.basename(fileName)
  };
}
...
var PATH = "link";
async function isDownloadableVideo(url2) {
  try {
    const res = await axios_default.head(url2, { timeout: 1e5 });
    const type = res.headers["content-type"];
    if (!type) return false;
    const videoTypes = [
      "video/",
      "application/octet-stream"
      // fallback (many servers use this)
    ];
...
function isFfmpeg(url2) {
  return new Promise((res, rej) => {
    getVideoInfo(url2).then(() => res(true)).catch(() => res(false));
    setTimeout(() => res(false), 9e4);
  });
}
