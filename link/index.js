        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFMPEG_PATH) {
              fs5.exists(process.env.FFMPEG_PATH, function(exists) {
                if (exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFPROBE_PATH) {
              fs5.exists(process.env.FFPROBE_PATH, function(exists) {
          ...
          function(ffprobe, cb) {
            ...
            self2._getFfmpegPath(function(err, ffmpeg4) {
              if (err) {
                ...
                fs5.exists(ffprobe2, function(exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FLVMETA_PATH) {
              fs5.exists(process.env.FLVMETA_PATH, function(exists) {
          ...
          function(flvtool, cb) {
            ...
            if (process.env.FLVTOOL2_PATH) {
              fs5.exists(process.env.FLVTOOL2_PATH, function(exists) {
      ...
      proto.takeScreenshots = proto.thumbnail = proto.thumbnails = proto.screenshot = proto.screenshots = function(config, folder) {
        ...
        async.waterfall([
          ...
          function createDirectory(filenames, next) {
            fs5.exists(config.folder, function(exists) {
              if (!exists) {
                fs5.mkdir(config.folder, function(err) {
                  if (err) {
                    next(err);
                  } else {
                    next(null, filenames);
...
function getTempName(orgPath) {
  const dir = import_path2.default.dirname(orgPath);
  const baseName = import_path2.default.basename(orgPath, import_path2.default.extname(orgPath));
  const newFileName = `${baseName}.temp${import_path2.default.extname(orgPath)}`;
  return import_path2.default.join(dir, newFileName);
}
async function continueDownloading(org, continued, start, duration) {
  const tempPath = getTempName(continued);
  import_fs2.default.renameSync(continued, import_path2.default.basename(tempPath));
...
function isFile2(link) {
  try {
    return import_fs3.default.statSync(link).isFile();
  } catch (error) {
    return false;
  }
}
var FfmpegBase = class _FfmpegBase extends LinkDownloadBase {
  ...
  async continuoDownloading(command) {
    ...
    commando.mergeToFile(transfer, import_path3.default.dirname(this.downloadingState.path));
    this.once("close", () => {
      try {
  ...
  async download(func) {
    ...
    const format = import_path3.default.extname(this.downloadingState.path).slice(1);
    this.setFileSize(
    ...
    if (this.downloadingState.continued && import_fs3.default.existsSync(this.downloadingState.path)) {
...
var FfmpegResizeBase = class _FfmpegResizeBase extends FfmpegBase {
  ...
  async continueDownResize(command) {
    ...
    commando.mergeToFile(
      this.downloadingState.path,
      import_path4.default.dirname(this.downloadingState.path)
    );
    this.once("close", () => {
      try {
        command.kill("SIGKILL");
...
async function getData(metadata, url2, fileName) {
  const videoFormat = import_path5.default.extname(fileName).slice(1);
      ...
      VIDEO: orgQuality && [
        {
          ...
          data: {
            ...
            title: import_path5.default.basename(fileName)
          },
      ...
      AUDIO: audioStream && [
        {
          ...
          data: {
            ...
            title: `${import_path5.default.basename(fileName)} - audioOnly`
          },
          quality: 0,
          text: {
      ...
      OTHERS: orgQuality && [
        {
          ...
          data: {
            ...
            ftype: videoFormat,
            fquality: orgQuality.label,
            title: `${import_path5.default.basename(fileName)} videoOnly`
          },
          quality: orgQuality.height,
          text: {
...
async function isDownloadableVideo(url2) {
  try {
    const res = await axios_default.head(url2, { timeout: 5e3 });
