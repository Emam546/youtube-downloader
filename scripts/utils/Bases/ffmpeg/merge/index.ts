...
export class FfmpegMergeBase extends FfmpegResizeBase {
    ...
    try {
      ...
      if (audioOnly) {
        ...
        if (data.resize && videoStream?.width) {
          const scaleFactor = data.resize / videoStream.width;
          overallBitrate =
            videoBitrate * Math.pow(scaleFactor, 2) + audioBitrate;
        }
      }
      const estimatedSize = (overallBitrate * EstimatedDuration) / 8;
      return isFinite(estimatedSize) && estimatedSize > 0
        ? estimatedSize
        : null;
    } catch (error) {
      console.error(error);
  ...
  async download(func: (path: string) => Writable) {
    ...
    if (
      ...
      await pipeAsync(stream.pipe.pipe(func(this.downloadingState.path)));
    } else {
