        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFMPEG_PATH) {
              fs8.exists(process.env.FFMPEG_PATH, function(exists) {
                if (exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FFPROBE_PATH) {
              fs8.exists(process.env.FFPROBE_PATH, function(exists) {
          ...
          function(ffprobe, cb) {
            ...
            self2._getFfmpegPath(function(err, ffmpeg6) {
              if (err) {
                ...
                fs8.exists(ffprobe2, function(exists) {
        ...
        async.waterfall([
          ...
          function(cb) {
            if (process.env.FLVMETA_PATH) {
              fs8.exists(process.env.FLVMETA_PATH, function(exists) {
          ...
          function(flvtool, cb) {
            ...
            if (process.env.FLVTOOL2_PATH) {
              fs8.exists(process.env.FLVTOOL2_PATH, function(exists) {
    ...
    module2.exports = function(proto) {
      proto.ffprobe = function() {
        ...
        this._getFfprobePath(function(err, path8) {
          ...
          var ffprobe = spawn(path8, ["-show_streams", "-show_format"].concat(options, src), { windowsHide: true });
          if (input.isStream) {
      ...
      proto.takeScreenshots = proto.thumbnail = proto.thumbnails = proto.screenshot = proto.screenshots = function(config, folder) {
        ...
        async.waterfall([
          ...
          function createDirectory(filenames, next) {
            fs8.exists(config.folder, function(exists) {
              if (!exists) {
                fs8.mkdir(config.folder, function(err) {
                  if (err) {
                    next(err);
                  } else {
                    next(null, filenames);
...
async function getFrameScreenShot(videoPath) {
  ...
  return await new Promise((res, rej) => {
    const tempThumbnailPath = import_path3.default.join(tempDir, `temp_thumbnail_${v4_default()}.png`);
    (0, import_fluent_ffmpeg.default)(videoPath).on("end", () => {
      ...
      filename: import_path3.default.basename(tempThumbnailPath),
      folder: import_path3.default.dirname(tempThumbnailPath),
      timestamps: [time],
      fastSeek: true
    });
  });
...
function getTempName(orgPath) {
  const dir = import_path4.default.dirname(orgPath);
  const baseName = import_path4.default.basename(orgPath, import_path4.default.extname(orgPath));
  const newFileName = `${baseName}.temp${import_path4.default.extname(orgPath)}`;
  return import_path4.default.join(dir, newFileName);
}
...
async function continueDownloading2(org, continued, start, duration) {
  const tempPath = getTempName(continued);
  import_fs4.default.renameSync(continued, import_path5.default.basename(tempPath));
  const info = await getVideoInfo(tempPath);
  const outputDuration = info.format.duration;
  const resumePoint = outputDuration > 5 ? outputDuration - 5 : 0;
...
function isFile2(link) {
  try {
    return import_fs5.default.statSync(link).isFile();
  } catch (error) {
    return false;
  }
}
var FfmpegBase = class _FfmpegBase extends LinkDownloadBase {
  ...
  async continuoDownloading(command) {
    ...
    commando.mergeToFile(transfer, import_path6.default.dirname(this.downloadingState.path));
    this.once("close", () => {
      try {
...
var FfmpegResizeBase = class _FfmpegResizeBase extends FfmpegBase {
  ...
  async continueDownResize(command) {
    ...
    commando.mergeToFile(
      this.downloadingState.path,
      import_path7.default.dirname(this.downloadingState.path)
    );
    this.once("close", () => {
      try {
        command.kill("SIGKILL");
